...
function init() {
  ...
  // Set reveal flag BEFORE setupInput (so it works even if input setup has issues)
  fixRevealImage();
  setupInput();
}
...
function showThankYou() {
  const reveal = document.getElementById('reveal-overlay');
...
function setupInput() {
  ...
  // Defensive button binding — works even if some elements are missing
  const nextBtn = document.getElementById('next-btn');
  if (nextBtn) nextBtn.addEventListener('click', showThankYou);

  const playAgainBtn = document.getElementById('play-again-btn');
  if (playAgainBtn) playAgainBtn.addEventListener('click', resetGame);

  const cornerReset = document.getElementById('corner-reset');
  if (cornerReset) cornerReset.addEventListener('click', resetGame);
}
